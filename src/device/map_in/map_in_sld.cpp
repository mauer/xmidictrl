 ...
 * Check the mapping
 */
bool map_in_sld::check(text_logger& in_log, const midi_device_settings& in_dev_settings)
{
	bool result = true;
	...
	if (!m_dataref.empty()) {
		...
		if (m_value_min == m_value_max) {
			in_log.error(source_line());
			in_log.error(
				fmt::format(" --> Parameter '{}' is equal to parameter '{}", c_cfg_value_min, c_cfg_value_max));
			result = false;
		}
	} else {
		// command mode
		if (m_command_up.empty() && m_command_down.empty()) {
			in_log.error(source_line());
			in_log.error(
				fmt::format(" --> Parameters '{}' and '{}' are not defined", c_cfg_command_up, c_cfg_command_down));
			result = false;
		}
	}
	...
	if (m_data_2_min >= m_data_2_max) {
		in_log.error(source_line());
		in_log.error(fmt::format(" --> Parameter '{}' ({:d}) has to be smaller than parameter '{}' ({:d})",
								 c_cfg_data_2_min,
								 m_data_2_min,
								 c_cfg_data_2_max,
								 m_data_2_max));
...
std::unique_ptr<map_result> map_in_sld::execute(map_param* in_param)
{
	...
	if (!m_dataref.empty()) {
		...
		if (param_in->msg().data_2() == m_data_2_min)
			...
			value =
				((m_value_max - m_value_min) * (static_cast<float>(param_in->msg().data_2()) / 127.0f)) + m_value_min;

		param_in->msg().log().debug(
			fmt::format(" --> Set dataref '{}' to value '{}'", m_dataref, std::to_string(value)));

		if (env().drf().write(param_in->msg().log(), m_dataref, value)) {
			try {
				m_label->display_label(param_in->msg().log());
			} catch (std::bad_alloc& ex) {
				param_in->msg().log().error(
					fmt::format("Error converting float '{}' value to string", std::to_string(value)));
 ...
 * Return the label id
 */
std::string map_in_sld::map_text_label()
{
	return m_label->id();
}
