 ...
 * Constructor
 */
map_in::map_in(app_services &in_app)
    : mapping(in_app)
{
}
...
{
    if (in_values.size() == 2) {
        auto value = app().datarefs().toggle(in_log, in_dataref, in_values[0], in_values[1]);
        display_label(in_log, value);
    } else {
        // get current value
        std::string value;
        app().datarefs().read(in_log, in_dataref, value);
        ...
        app().datarefs().write(in_log, in_dataref, value);
        display_label(in_log, value);
    }
}
