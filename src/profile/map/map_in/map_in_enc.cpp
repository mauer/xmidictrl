 ...
 * Constructor
 */
map_in_enc::map_in_enc(app_services &in_app)
    : map_in(in_app)
{}
...
bool map_in_enc::execute_dataref(midi_message &in_msg)
{
    ...
    // read current value
    if (!app().datarefs().read(in_msg.log(), m_dataref, value)) {
    ...
    if (m_mode == encoder_mode::relative) {
        ...
        if (in_msg.data_2() < 64) {
            ...
            if (in_msg.data_2() < 61) {
                ...
                app().cmd().execute(in_msg.log(), m_command_fast_down);
            } else {
            ...
            if (in_msg.data_2() > 68) {
                ...
                app().cmd().execute(in_msg.log(), m_command_fast_up);
            } else {
