	...
	: xplane_window(in_log, in_env, in_width, in_height, in_position, in_offset_x, in_offset_y, in_translucent)
{
	...
	if (!s_font) {
		...
		builder.AddText(
			ICON_FA_ARROW_LEFT ICON_FA_ARROW_RIGHT ICON_FA_ARROW_RIGHT_FROM_BRACKET ICON_FA_ARROW_RIGHT_TO_BRACKET
				ICON_FA_SQUARE_XMARK ICON_FA_TRIANGLE_EXCLAMATION ICON_FA_CIRCLE_CHECK ICON_FA_CHECK
					ICON_FA_CHECK_DOUBLE ICON_FA_INFO ICON_FA_TRASH_CAN ICON_FA_FLOPPY_DISK ICON_FA_ROTATE_RIGHT);
...
void imgui_window::on_key(char in_key, XPLMKeyFlags in_flags, char in_virtual_key, bool in_losing_focus)
{
	...
	if (io.WantTextInput) {
		...
		if ((in_flags & xplm_UpFlag) != xplm_UpFlag && !io.KeyCtrl && !io.KeyAlt && std::isprint(in_key)) {
